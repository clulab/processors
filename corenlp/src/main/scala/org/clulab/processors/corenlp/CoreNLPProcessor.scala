  ...
  override def parse(doc:Document) {
    ...
    for (sa <- sas) {
      ...
      if(! depFail) {
        ...
        doc.sentences(offset).setDependencies(GraphMap.UNIVERSAL_BASIC, CoreNLPUtils.toDirectedGraph(basicDeps, in, Some(words.size())))
        doc.sentences(offset).setDependencies(GraphMap.UNIVERSAL_ENHANCED, CoreNLPUtils.toDirectedGraph(enhancedDeps, in, Some(words.size())))
      }
