...
`m.t.type` | Type of task to be learned. "basic" indicates any task that is represented in the two-column format described above. See the Semantic Role Labeling section to see an example of a different task. | "basic"
...
`m.t.t.inference` | Type of inference to be used for this layer: "greedy" implements a left-to-right greedy prediction. "viterbi" implements a CRF layer. | "greedy"
