...
abstract class BiMEMMSequenceTagger[L: ClassTag, F: ClassTag](
  ...
  override def train(docs:Iterator[Document]): Unit = {
    ...
    FeatureExtractor.countBigrams(sentences.toSeq, FeatureExtractor.BIGRAM_THRESHOLD)
  ...
  def classesOf(classifier: Classifier[L, F],
    ...
    for(i <- 0 until sentence.size) {
      ...
      mkFeatures(feats, sentence, i, history.toSeq, firstPass)
      val d = mkDatum(null.asInstanceOf[L], feats)
      val label = classifier.classOf(d)
      history += label
    }

    if(leftToRight) history.toArray else SeqUtils.revert(history.toSeq).toArray
  }
