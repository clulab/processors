        ...
        if(testSentences.nonEmpty){
            ...
            for (sent <- testSentences) {
                ...
                for(as <- annotatedSentences) {
                    ...
                    for(i <- words.indices){
                        val word = words(i)
                        embeddings(i) = wordEmbeddingMap.getOrElse(word, 0).get.toArray
                        wordIds(i) = w2i.getOrElse(word, 0).toLong
                        val char_input = new java.util.HashMap[String, OnnxTensor]()
                        char_input.put("char_ids",  OnnxTensor.createTensor(ortEnvironment, word.map(c => c2i.getOrElse(c.toString, 0).toLong).toArray))
