...
object EvalEisner extends App {
  ...
  Synchronizer.withComputationGraph("EvalEisner") { implicit cg =>
    ...
    for (sentence <- sentences) {
      ...
      for (as <- annotatedSentences) {
        ...
        val preds = eisner.ensembleParser(heads, Some(labels), annotatedSentence, constEmbeddings, 5, 0.5f, true)
