...
class TestArray extends Test {
  ...
  it should "work as expected" in {
    ...
    val wrapped2a = processSeq(wrapped2)
    val wrapped2b = processIndexedSeq(wrapped2)

    wrapped2.toArray.eq(array) should be (true)
    wrapped2a.toArray.eq(array) should be (true)
    wrapped2b.toArray.eq(array) should be (true)
  }
}
