    ...
    override def mkThreads(
      ...
      def loop(
        ...
        internals match {
          ...
          case (headInst, headGroups, headMentions, headPartialGroups) :: rest => {
            headInst match {
              case inst: Pass =>
...
//        debugger.debugMatches(false) // This would be for the thread then?
    ...
    override def stepSingleThread(t: SingleThread): Seq[Thread] = {
      printThread(t)
    ...
    override def handleDone(threads: Seq[Thread]): (Seq[Thread], Option[Thread]) = {
      ...
      def dubugDoneThread(thread: Thread): Unit = {
        thread match {
          ...
          case threadBundle: ThreadBundle =>
            threadBundle.bundles.foreach { bundle =>
              bundle.foreach { thread =>
                dubugDoneThread(thread)
              }
            }
        }
      }

      threads.foreach(dubugDoneThread)
