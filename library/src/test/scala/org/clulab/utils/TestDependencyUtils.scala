...
class TestDependencyUtils extends Test {
  ...
  it should "produce 'substrates' as the head of 'the substrates of Shp2'" in {
    // 3 is a root, so be sure to avoid it in the former interval (1, 5).
    val result = findHeadStrict(Interval(1, 3), sent1)
    result shouldBe defined
    result.get should be (2)
  }
  it should "produce 'are' as the head of 'the substrates of Shp2 are'" in {
    // 3 is a root, so be sure to avoid it in the former interval (1, 6).
    val result = findHeadStrict(Interval(4, 6), sent1)
    result shouldBe defined
  ...
  it should "have the same getHeadStrict as roots" in {
    val head = findHeadStrict(Interval(0, 20), sent2).get
    // There are multiple, unsorted roots, so a simple head is not sufficient.
    val roots = sent2.dependencies.get.roots
    roots should contain (head)
  }
  ...
  val text3 = "."
  val doc3 = jsonStringToDocument(""" {"sentences":[{"words":["."],"startOffsets":[0],"endOffsets":[1],"tags":["."],"lemmas":["."],"entities":["O"],"norms":["O"],"chunks":["O"],"graphs":{"universal-basic":{"edges":[],"roots":[0]},"universal-enhanced":{"edges":[],"roots":[0]}}}]} """)
  val sent3 = doc3.sentences.head

  behavior of text3

  it should "produce one head using findHeads" in {
    val heads = findHeads(Interval(0, 1), sent3.dependencies.get)

    heads should have size (1)
    distToRootOpt(Interval(0, 1), sent3.dependencies.get) should be (Some(0))
  }
  it should "produce no heads using findHeadsStrict" in {
    findHeadsStrict(Interval(0, 1), sent3) should have size (0)
  }

  behavior of "DependencyUtils"

  it should "handle cycles in the dependencyGraph correctly" in {
  ...
  it should "handle invalid dependency trees in findHeads" in {
    val edges = List(
